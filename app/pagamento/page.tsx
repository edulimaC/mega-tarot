...
export default function PaymentPage() {
  ...
  return <main className="payment-page">
    ...
    <section className="payment-shell" aria-live="polite">
      {showCheckout && <>
        <div className="payment-copy"><p className="eyebrow-light">a sua mesa está pronta</p><h1>Desbloqueie<br /><em>o que as cartas viram.</em></h1><p>O resultado foi construído a partir das suas 12 escolhas. Faça o Pix para revelar as cinco cartas e a mensagem central da sua leitura.</p><div className="payment-offer" aria-label={`Oferta: de R$ 18,99 por R$ ${payment.amount.toFixed(2).replace(".", ",")}`}><span className="payment-offer-name">{payment.offerName}</span><div className="payment-price"><span className="payment-price-old">De <s>R$ 18,99</s></span><span className="payment-price-label">por apenas</span><strong>R$ {payment.amount.toFixed(2).replace(".", ",")}</strong><span className="payment-price-note">pagamento único via Pix</span></div></div></div>
